...
contract Payment is IPayment, AccessControl, ReentrancyGuard, Pausable {
    ...
    function processPrimaryPurchase(
        address organizer,
        uint256 price
    ) external payable override nonReentrant whenNotPaused onlyTicketContract returns (bool) {
    ...
    function processSecondaryPurchase(
        address seller,
        uint256 price
    ) external payable override nonReentrant whenNotPaused onlyTicketContract returns (bool) {
        require(msg.value >= price, "Insufficient payment");
    ...
    function processRefund(
        address to,
        uint256 amount
    ) external override nonReentrant onlyTicketContract returns (bool) {
    ...
    function setPlatformFee(
        uint256 newFee
    ) external override onlyRole(FEE_MANAGER_ROLE) validFee(newFee) {
        platformFee = newFee;
    ...
    // Internal functions
    function _calculatePaymentDetails(
        uint256 amount,
        PaymentType paymentType
    ) internal view returns (PaymentDetails memory) {
